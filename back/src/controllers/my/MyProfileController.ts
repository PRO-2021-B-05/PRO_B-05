...
export class MyProfileController {
    ...
    @Put('/')
    @Returns(200, Student).Groups("user.show")
    async put(@Req() req: Request, @BodyParams(Student) @Groups("user.update") student: Partial<Student>) {
        ...
        return deserialize(await this.studentRepository.findOne({uuid: existingStudent.uuid}), {type: Student, groups: ['user.show']});
    }
